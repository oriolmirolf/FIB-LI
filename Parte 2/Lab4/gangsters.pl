...
satVariable( gangsterWorksAtHour(G, H)) :- gangster(G), hour(H).
...
writeClauses(MaxConsecutiveHours):-
    lligarVariables,
    gangsterOnVacation,
    eachGangsterEachHourAtMostOneTask,
    allTasksDone,
    noMoreThan(MaxConsecutiveHours),
    noDiffConsec,
    true,!.
...
noMoreThan(MaxConsecutiveHours) :-
        ...
        % Aqui mirem que per cada cadena possible 
        % de longitud MaxConsecutiveHours,
        % No es compleixi al menys 1.
        % Per exemple, 
        %   - Si MaxConsecutiveHours = 5
        %   - Potencialment pogues treballar les
        %       hores h1, h2, h3, h4, h5
        %   Tindriem que atLeast(1, (-h1, ..., -h5)
        %   Per tal de que no es poguin complir totes.
