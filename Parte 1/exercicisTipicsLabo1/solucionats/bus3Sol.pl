...
cities(L):- setof(X, (traject(X,_,_,_);traject(_,X,_,_)), L).
...
%la funcion de abajo solo hace (con la definicion dada del profe) caminos con diferente inicio y destino.
%Pero no tiene en cuenta que la entrada puede ser un viaje de ida y vuelta (ejemplo bcn-mad-bcn) 
...
satVariable(dcc(D,C1,C2)):- day(D), trip(C1-C2). %en un dia D se hace C1-C2.
%SsatVariable(dbc(D,B,C)):- day(D), bus(B), city(C).
...
writeClauses:-  
    ...
    exactlyOneBusPerTripAndDay,
    bdccNdccRelation,
    frequencyRestriction,
    distanceRestriction,
    startWhereEndRestriction,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactlyOneTripPerBusAndDay:- 
    day(D), bus(B), 
    findall(bdcc(B,D,C1,C2), (trip(C1-C2)) ,Set), 
    exactly(1, Set), fail.
exactlyOneTripPerBusAndDay.

exactlyOneBusPerTripAndDay:- 
    trip(C1-C2), day(D), 
    findall(bdcc(B,D,C1,C2), bus(B), Set), 
    atMost(1, Set), fail.
exactlyOneBusPerTripAndDay.
...
bdccNdccRelation:- 
    day(D), trip(C1-C2), 
    findall(bdcc(B,D,C1,C2),bus(B), Set), 
    expressOr(dcc(D,C1,C2), Set),
    fail.
bdccNdccRelation.

frequencyRestriction:- 
    trip(C1-C2), frequency(C1-C2, F), 
    findall(dcc(D,C1,C2), day(D), Set), 
    atLeast(F,Set), 
    fail.
frequencyRestriction.

distanceRestriction:- 
    day(D), bus(B), tooLongDist(C1-C2-C3),
    consecutiveDays(D, D1),
    writeOneClause([-bdcc(B,D,C1,C2), -bdcc(B,D1,C2,C3)]), 
    fail.
distanceRestriction.

% D1-D2, bus(B), C1-C2, C3-C4,
% C2 \= C3
% -(bdcc(B, D1, C1, C2) & bdcc(B, D2, C3, C4))

startWhereEndRestriction:- 
    day(D), bus(B), trip(C1-C2), trip(C3-C4), 
    C2 \= C3,consecutiveDays(D, D1), 
    writeOneClause([-bdcc(B,D,C1,C2),-bdcc(B,D1,C3,C4)]), 
    fail.
startWhereEndRestriction.
