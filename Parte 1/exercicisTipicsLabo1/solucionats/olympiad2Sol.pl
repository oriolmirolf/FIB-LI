...
subj(S):-findall(K,expertise(_,K), L1),sort(L1,Set) ,member(S,Set).
...
exactlyOne:- findall(subject(S), subj(S), Set), numMembersTeam(K),exactly(K,Set).
exactlyOneSubject:- subj(S), findall(chosen(T), expertise(T,S), Set), atMost(1,Set), fail.
exactlyOneSubject.

relacion:- subj(S), findall(chosen(T), expertise(T,S), Set), expressOr(subject(S),Set), fail.
relacion.

sameteam:- student(S), student(S1), S1 \= S , not(friends(S,S1)), writeOneClause([-chosen(S), -chosen(S1)]), fail.
sameteam.
