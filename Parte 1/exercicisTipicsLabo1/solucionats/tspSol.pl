...
adjacentCities(C1, C2) :- adjacency(C1, L), member(C2, L).
...
satVariable( consecutive(C1, C2)) :- city(C1), city(C2).
...
writeClauses:- 
      ...
      eachCityVisitedExactlyOnce,
      ligarVisitedConsecutive,
      costIsOkay,
      cityOneIsFirstAndLast.

writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.


correctOrder :-
      %Can only visit city C in position P if you visited a city that connects to C in position P-1
      % if C1 cannot go to C2, -(visited(C1, P1) & visited(C2, P))
      % -visited(C1, P1) v -visited(C2, P)

      position(P), P > 0, P1 is P - 1,
      city(C1), city(C2),
      not(adjacentCities(C1, C2)),
      writeOneClause( [-visited(C1, P1), -visited(C2, P)] ),
      fail.
correctOrder.
...
ligarVisitedConsecutive :-
      % visited(C1, P) & visited(C2, P+1) -> consecutive(C1, C2)
      % -visited(C1, P) v -visited(C2, P+1) v consecutive(C1, C2)
      numCities(N),
      city(C1), city(C2), position(P), P < N,
      P1 is P + 1,
      writeOneClause( [-visited(C1, P), -visited(C2, P1), consecutive(C1, C2)]),
      fail.
ligarVisitedConsecutive.

costIsOkay :-
      % La cantidad de ciudades consecutivas con id par < max
      findall(consecutive(C1,C2), (city(C1), city(C2), ((C1 mod 2) ==(C2 mod 2))), Lits),
      maxCost(K),
      atMost(K, Lits),
      fail.
costIsOkay.
...
displaySol(M):- position(P), member(visited(City,P),M), write(City), nl, fail.
displaySol(_):- nl.
