...
satVariable( bdc(B,D,C) )       :- bus(B), day(D), city(C). % Bus B on day D ends is in city C.
...
lligarVar1Var2 :-
    ...
    findall(bdcc(B, D, C1, C2), )
    writeOneClause( [bdcc(_, D, C1, C2), -dcc(D, C1, C2)]),

    % bdcc(B, D, C1, C2) <-> dcc(D, C1, C2)
    % bdcc(B, D, C1, C2) -> dcc(D, C1, C2) & 
    % bdcc(B, D, C1, C2) <- dcc(D, C1, C2)
    % 
    fail.
