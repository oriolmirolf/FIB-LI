...
incompatibleWorkers(W1,W2):- workerScore(W1, S1), workerScore(W2, S2), maxScore(MS), W1 < W2,
                        Score is S1 + S2, Score > MS.  %% Complete this!
...
eachWexactlyOneT :- 
      worker(W), 
      findall(wt(W,T), team(T), L), exactly(1, L), fail.
eachWexactlyOneT.
